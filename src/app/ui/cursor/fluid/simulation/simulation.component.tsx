...
import { generateColor } from "../fluid.utils";
...
export const FluidSimulation = ({
  ...
  const engine = useMemo(() => {
    ...
    const simW = Math.round(
      props.simResolution * (aspectRatio > 1 ? aspectRatio : 1),
    );
    const simH = Math.round(
      props.simResolution * (aspectRatio < 1 ? 1 / aspectRatio : 1),
    );
    const dyeW = Math.round(
      props.dyeResolution * (aspectRatio > 1 ? aspectRatio : 1),
    );
    const dyeH = Math.round(
      props.dyeResolution * (aspectRatio < 1 ? 1 / aspectRatio : 1),
    );
    ...
    const fbos = {
      ...
      divergence: new THREE.WebGLRenderTarget(simW, simH, {
        type: texType,
      ...
      curl: new THREE.WebGLRenderTarget(simW, simH, {
        type: texType,
        format: THREE.RedFormat,
        minFilter: THREE.NearestFilter,
        magFilter: THREE.NearestFilter,
  ...
  useEffect(() => {
    return () => {
      ...
      Object.values(engine.materials).forEach((m) => {
        m.dispose();
      });
    };
  }, [engine]);
  ...
  useFrame(({ gl }, delta) => {
    ...
    if (pointer.current.moved) {
      ...
      splatMat.uniforms.point.value.set(
        pointer.current.texcoordX,
        pointer.current.texcoordY,
      );
      splatMat.uniforms.color.value.set(
        pointer.current.deltaX * props.splatForce,
        pointer.current.deltaY * props.splatForce,
        0,
      );
      splatMat.uniforms.radius.value =
        (props.splatRadius / 100) *
        (engine.aspectRatio > 1 ? engine.aspectRatio : 1);
      ...
      splatMat.uniforms.color.value.set(
        pointer.current.color.r,
        pointer.current.color.g,
        pointer.current.color.b,
      );
    ...
    // 2. Curl
    engine.materials.curl.uniforms.uVelocity.value =
      engine.fbos.velocity.read.texture;
    blit(engine.fbos.curl, engine.materials.curl);

    // 3. Vorticity
    engine.materials.vorticity.uniforms.uVelocity.value =
      engine.fbos.velocity.read.texture;
    ...
    // 4. Divergence
    engine.materials.divergence.uniforms.uVelocity.value =
      engine.fbos.velocity.read.texture;
    blit(engine.fbos.divergence, engine.materials.divergence);

    // 5. Clear Pressure
    engine.materials.clear.uniforms.uTexture.value =
      engine.fbos.pressure.read.texture;
    ...
    engine.materials.pressure.uniforms.uDivergence.value =
      engine.fbos.divergence.texture;
    for (let i = 0; i < props.pressureIterations; i++) {
      engine.materials.pressure.uniforms.uPressure.value =
        engine.fbos.pressure.read.texture;
    ...
    // 7. Gradient Subtract
    engine.materials.gradientSubtract.uniforms.uPressure.value =
      engine.fbos.pressure.read.texture;
    engine.materials.gradientSubtract.uniforms.uVelocity.value =
      engine.fbos.velocity.read.texture;
    blit(engine.fbos.velocity.write, engine.materials.gradientSubtract);
    engine.fbos.velocity.swap();

    // 8. Advection (Velocity)
    engine.materials.advection.uniforms.uVelocity.value =
      engine.fbos.velocity.read.texture;
    engine.materials.advection.uniforms.uSource.value =
      engine.fbos.velocity.read.texture;
    engine.materials.advection.uniforms.dissipation.value =
      props.velocityDissipation;
    engine.materials.advection.uniforms.dt.value = dt;
    engine.materials.advection.uniforms.texelSize.value =
      engine.uniforms.simTexelSize;
    ...
    engine.materials.display.uniforms.uTexture.value =
      engine.fbos.dye.read.texture;
